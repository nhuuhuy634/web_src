...
//find customer name
router.get("/find-customer/:username", JwtUtil.checkToken, async (req, res) => {
  const { username } = req.params;
  const customers = await CustomerDAO.findByUsername(username);
...
//find category
router.get("/find-category/:category", JwtUtil.checkToken, async (req, res) => {
  const { category } = req.params;
  const categories = await CategoryDAO.findByName(category);
